...
export class ComparablesComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    ngOnInit(): void {
      this.route.params.subscribe((params) => {
        ...
        [this.licenseeName, this.licensorName] = dynamicTitle.split('-'); // Assign values to licenseeName and licensorName
  ...
  createBoxPlotChart(scale: string, licenseeName: string = '', licensorName: string = ''): void {
    ...
    if (scale === '%') {
      ...
      const estimateData = [95, 115, 135, 155, 175];
      ...
      data = [
        ...
        },
        {
          ...
          type: 'box',
          name: 'Comparable licenses',
        },
        {
          x: normalizeData(estimateData),
          type: 'box',
          name: 'Estimate',
        }
      ...
      const estimateData = [0.1, 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]; 
      ...
      const estimateData = [2500000, 3500000, 4500000, 5500000, 6500000, 7500000, 8500000, 9500000, 10500000];
    ...
    const separatorY = data.findIndex(entry => entry.name === 'Comparable licenses') + 0.5;
      ...
      shapes: [
        {
          ...
          x1: 1,
          yref: 'y',
          y0: data.findIndex(entry => entry.name === 'Comparable licenses') + 0.5,
          y1: data.findIndex(entry => entry.name === 'Comparable licenses') + 0.5,
          line: {
            color: 'black',
            width: 2,
            dash: 'solid',
      ...
      annotations: [
        {
          ...
          yref: 'y',
          y: data.findIndex(entry => entry.name === 'Comparable licenses') + 0.5,
   ...
   let hoverFlagComparable = false; // Flag to track hover event for Comparable licenses
   ...
   let hoverFlagEstimate = false;

    Plotly.newPlot('myDiv', data, layout, { displaylogo: false }).then((chart: any) => {
      chart.on('plotly_click', (eventData: any) => {
        // Check if the click event occurred in the 'Comparable licenses' box
        const clickedName = eventData.points[0].data.name;

        if (clickedName === 'Comparable licenses' && hoverFlagComparable) {
          // Save 'licensorName' to local storage when clicking on 'Comparable licenses'
          this.saveVersusNameToLocalStorage(this.licensorName);
          this.saveCompanyNameToLocalStorage(this.licenseeName)
          // Navigate to the SpiderChartComponent for Comparable licenses
          ...
          // Save 'licensorName' to local storage when clicking on 'Estimate'
          this.saveVersusNameToLocalStorage(this.licensorName);
          this.saveCompanyNameToLocalStorage(this.licenseeName)
     ...
     chart.on('plotly_hover', (eventData: any) => {
       ...
       }   else if (hoveredName === 'Estimate') {
          hoverFlagComparable = false;
          hoverFlagTopDown = false;
          hoverFlagBottomDown = false;
          hoverFlagEstimate = true;
